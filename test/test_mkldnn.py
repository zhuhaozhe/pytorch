    ...
    def test_lstm(self):
        ...
        for dtype in types:
            ...
            for input_size, hidden_size, num_layers, bidirectional, bias, batch_first, dropout, batch_size, seq_len, training \
                ...
                with torch.amp.autocast('cpu', enabled=bf16 or fp16, dtype=lowp_dtype), torch.no_grad() if not training else nullcontext():
                    with torch.backends.mkldnn.flags(enabled=False):
                        torch.manual_seed(seed)
                        output1, (hn1, cn1) = self._cast_dtype(model1, bf16, fp16)(self._cast_dtype(input1, bf16, fp16),
                    ...
                    if training:
                        ...
                        for name, para in model1.named_parameters():
                            ...
                            self.assertEqual(para.grad, self._cast_dtype(getattr(model2, name).grad, bf16, fp16), rtol=rtol, atol=atol)
